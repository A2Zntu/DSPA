...
template<class ItemType>
LinkedSortedList<ItemType> LinkedSortedList<ItemType>::operator+(const LinkedSortedList& anotherList) {
	LinkedSortedList<ItemType> new_SortedList; 
	if(isEmpty())
		return anotherList; 
	
	new_SortedList = *this; 
	Node<ItemType>* right_chainPtr = anotherList.headPtr; 
	
	while(right_chainPtr != nullptr){
		new_SortedList.insertSorted(right_chainPtr -> getItem()); 
		right_chainPtr = right_chainPtr->getNext(); 
