...
class Path {
	private:
		vector<int> nodes;
		int capacity;
		int lastNodeCapacity; 
	public:
		Path();
		Path(int aNode);
		Path(Path aShorterPath, int aNode, int capacity, int lastNodeCap);
		int getTail();
		int getCapacity();
		int getLastNodeCapacity(); 
		void print();
		void reset();
		int getNodeSize();
...
Path::Path(int aNode) : capacity(VERY_LARGE), lastNodeCapacity(VERY_LARGE) {
	this->nodes.push_back(aNode);
}

Path::Path(Path aShorterPath, int aNode, int capacity, int lastNodeCap) {
	for(int i = 0; i < aShorterPath.nodes.size(); i++)
		this->nodes.push_back(aShorterPath.nodes[i]);
	this->nodes.push_back(aNode);
	this->capacity = capacity;
	this->lastNodeCapacity = lastNodeCap; 
}
// constructor of Path
...
void Path::print() {
	if(this->nodes.size() == 0)
		...
		cout << int2str(this->nodes.back()) << ";" << endl;
		cout << "Path Capacity: " << this->capacity << endl;
		cout << "Path LastNodeCapacity: " << this->lastNodeCapacity << endl;
	}
}
...
int Path::getLastNodeCapacity(){
	return this->lastNodeCapacity; 
}
...
bool Network::sortByRules(Path& lhs, Path& rhs) {
	if(lhs.getLastNodeCapacity() == rhs.getLastNodeCapacity())
		return stringCompare(lhs.getTail(),  rhs.getTail());
	else
		return lhs.getLastNodeCapacity() > rhs.getLastNodeCapacity();
}
...
int main() {
	...
	Network emptyNetwork(n);
	int maxFlowCnt = net.maxFlow(emptyNetwork);
