...
int main() {
	...
	for (int i = 0; i< 5; i++) {
		...
		Beta = line.substr(last, next-last);
		int beta = stoi(Beta);
		last = next + 1;
		switch(i) {
			case 0:
	...
	cin >> shipFileName >> taskFileName >> priorityName;
	cout << "beta1: " << coef.beta1 << endl
	     << "beta2: " << coef.beta2 << endl
	     << "beta3: " << coef.beta3 << endl
	     << "beta4: " << coef.beta4 << endl;
