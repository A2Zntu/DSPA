...
class Task {
	private:
		...
		int realStartTime;
	public:
		Task(Ship* sPtr, string sTime, int wTime, string type, int param1, int param2);
		//get ship info
		string getShipName() {
		...
		int getRealEndTime() {
			return realStartTime + workingTime;
		}
		int getWaitingTime() {
...
//constructor
Task::Task(Ship* sPtr, string sTime, int wTime, string aType, int param1, int param2) {
...
void Task::printTaskInfo() {
	cout << shipPtr->getShipName() << "\t" << parseTime(startTime) << "\t" << workingTime << "\t" << type << "\t";
	cout << parseTime(realStartTime) << "\t" << parseTime(realStartTime + workingTime) << "\t" << realStartTime- startTime << endl;
	/*if(type=="I")
	{
...
class TaskList {
	...
	public:
		...
		~TaskList();
		//void addTask()
		void addTask(Ship* shipPtr, string sTime, int wTime, string type, int param1, int param2);
...
void TaskList::addTask(Ship* shipPtr, string sTime, int wTime, string type, int param1, int param2) {
	if(taskNum<taskCapacity) {
		Task* newTask = new Task(shipPtr, sTime, wTime, type, param1, param2);
		taskList[taskNum] = newTask;
		taskNum++;
	}
}

//wrong ERT algorithm (can't handle task with 0 working time)
int TaskList::getERTDelayTime() {
	...
	int totalWaiting = 0;
	int prevPort1EndTime = 0;
	int prevPort2EndTime = 0;
	for(int i=0; i<taskNum; i++) {
		if((sortedList[i]->getType()=="I") || (sortedList[i]->getType()=="O")) {
			//only inport and outport need to count
			switch (sortedList[i]->getPort()) {
				case 1 :
					//port 1
					if(sortedList[i]->getStartTime() < prevPort1EndTime) {
						//need to wait
						totalWaiting += prevPort1EndTime - sortedList[i]->getStartTime();
						prevPort1EndTime += sortedList[i]->getWorkTime();
					} else
						prevPort1EndTime = sortedList[i]->getStartTime() + sortedList[i]->getWorkTime();
					break;
				case 2 :
					//port 1
					if(sortedList[i]->getStartTime() < prevPort2EndTime) {
						//need to wait
						totalWaiting += prevPort2EndTime - sortedList[i]->getStartTime();
						prevPort2EndTime += sortedList[i]->getWorkTime();
					} else
						prevPort2EndTime = sortedList[i]->getStartTime() + sortedList[i]->getWorkTime();
					break;
			}
...
int TaskList::getERT2DelayTime() {
	...
	for(int i=0; i<taskNum; i++) {
		...
		if(portNum == 1 && port1Count== 0) {
			curTask->setRealStartTime(curTask->getStartTime());
		...
		//non empty
			...
			//head: waiting time = 0
			if(curTask->getStartTime() + curTask->getWorkTime() <= port1Task[0]->getRealStartTime()) {
				//move and insert
				...
				for(int j=1; j<port1Count; j++) {
					//
					if(max(curTask->getStartTime() ,port1Task[j-1]->getRealEndTime()) + curTask->getWorkTime() <= port1Task[j]->getRealStartTime()) {
						...
						//insert
						curTask->setRealStartTime(max(curTask->getStartTime(), port1Task[j-1]->getRealEndTime()));
						port1Task[j] = curTask;
				...
				if(!isFound) {
					//insert
					curTask->setRealStartTime(max(curTask->getStartTime(), port1Task[port1Count-1]->getRealEndTime()));
					port1Task[port1Count] = curTask;
			...
			//head: waiting time = 0
			if(curTask->getStartTime() + curTask->getWorkTime() <= port2Task[0]->getRealStartTime()) {
				//move and insert
				...
				for(int j=1; j<port2Count; j++) {
					//find enough space
					if(max(curTask->getStartTime() ,port2Task[j-1]->getRealEndTime()) + curTask->getWorkTime() <= port2Task[j]->getRealStartTime()) {
						isFound = true;
						pos = j;
						break;
					}
				...
				//insert
				curTask->setRealStartTime(max(curTask->getStartTime(), port2Task[pos-1]->getRealEndTime()));
		...
		if(ertDebugMode) {
			...
			cout << "NAME" << "\t" << "startT" << "\t" << "workT" << "\t" << "type" << "\t" << "realT" << "\t" << "endT" << "\t" << "waitT" << endl;
...
int main() {
	...
	while(getline(infile, line)) {
		...
		//workingTime
		pos = line.find(delimiter);
		workingTime = stoi(line.substr(0, pos));
		line.erase(0, pos + delimiter.length());
		//type
		pos = line.find(delimiter);
		...
		taskList.addTask(shipPtr, startTime, workingTime, type, param1, param2);
	}
	infile.close();
	taskList.printTasks();
